...
class CustomUserAdmin(UserAdmin):
    ...
    list_editable = ('is_active', 'is_staff', 'is_superuser')  # Разрешить редактирование статусов
...
class SupplierAdmin(admin.ModelAdmin):
    list_display = ('name', 'country', 'city', 'street', 'building')  # Поля для отображения в списке
    search_fields = ('name', 'city')  # Поля для поиска
    list_filter = ('country', 'city')  # Фильтры
...
class UserProfileAdmin(admin.ModelAdmin):
    list_display = ('user', 'first_name', 'last_name', 'email_verified', 'age')
    search_fields = ('user__username', 'first_name', 'last_name')
    list_filter = ('email_verified',)  # Добавляем фильтр по статусу
    list_editable = ('email_verified',)  # Разрешить редактирование email_verified